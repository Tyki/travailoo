 ...
 */
class HttpProtocol extends Base_1.KuzzleAbstractProtocol {
    /**
        ...
        for (const controller of Object.keys(this._customRoutes)) {
            ...
            for (const action of Object.keys(definition)) {
                ...
                if (!(typeof route.url === "string" && route.url.length > 0)) {
                    throw new Error(`Incorrect URL for custom route ${controller}:${action}.`);
                }
                if (!(typeof route.verb === "string" && route.verb.length > 0)) {
                    throw new Error(`Incorrect VERB for custom route ${controller}:${action}.`);
                }
            }
        }
    }
    ...
    connect() {
        ...
        return this._sendHttpRequest(publicApiRequest)
            ...
            if (error.status === 401 || error.status === 403) {
                ...
                this._routes = routes_json_1.default;
                return;
            }
                ...
                return this._sendHttpRequest(serverInfoRequest).then(({ result: res, error: err }) => {
                    if (!err) {
    ...
    _sendHttpRequest({ method, path, payload, }) {
        ...
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest(), url = `${this.protocol}://${this.host}:${this.port}${path}`;
